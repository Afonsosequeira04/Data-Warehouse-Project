...
- Current phase: **Fase 3 (migração para dbt) — in progress on branch `fase-3-dbt-migration`.**
...
Guardrails:
- This must be the **last** commit of the branch. The line reaches `main` only when the owner merges the PR.
...
- **Repo layout:**
  ...
  - `legacy_sql/` — legacy `silver/` and `gold/` scripts
...
```
If any of the dbt targets above is missing, Fase 3 is not finished.
